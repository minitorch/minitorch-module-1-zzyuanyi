...
@pytest.mark.task0_2
@given(small_floats)
def test_sigmoid(a: float) -> None:##############################zanshiyouwenti
    ...
    * It is  strictly increasing.
    ...
    assert 1.0 - sigmoid(a) == pytest.approx(sigmoid(-a), abs=1e-6), "对称性不成立"
    assert 0.5==sigmoid(0)
    assert sigmoid(a)<=sigmoid(a+1e-2)#zheli maosi youdian wenti a 
